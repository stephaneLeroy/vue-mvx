...
import {computed, defineEmits, defineProps, nextTick, onMounted, watch, watchEffect} from "vue";
...
const isMobile = computed(() => {
    console.log(platform, platform.os?.family === "iOS")
...
const login = async () => {
    ...
    const loginData: MaiarAppLoginData | undefined = await erd.maiarApp.login({token: props.token});
    if(!loginData) {
        console.log("No Maiar App login data available")
        return;
    }
...
watch(() => props.selectedMode,  async (selectedMode) => {
    if(selectedMode === 'Maiar') {
        openContent.value = true;
        await nextTick(login);
    } else {
